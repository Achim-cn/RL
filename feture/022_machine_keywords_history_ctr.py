...
train = train[['instance_id', 'feedid', 'device', 'machine_keyword_list', 'date_'] + y_cols].copy()
...
for ref_date in tqdm(total_periods):
    ...
    tmp_label = tmp_label.explode(key).reset_index(drop=True)
    history_log = history_log.explode(key).reset_index(drop=True)
...
features_df.to_pickle('./features/tr_machine_keyword_list_hist_ctr.pkl')
